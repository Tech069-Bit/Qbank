...
// ADDED: tapping a notification raised via registration.showNotification() (see
// fireSystemNotification() in index.html — Study Mode phase-change alerts and Study Planner
// block reminders both go through it) did nothing by default: the notification just sat there
// and dismissed itself on tap without ever bringing the app to the front. This focuses an
// already-open QBank Center tab if one exists, or opens a new one otherwise, then closes the
// notification either way so it doesn't linger in the tray after being acted on.
  ...
  event.waitUntil(
    (async () => {
      ...
      for (const client of allClients) {
        // Same-origin tab already open (any path within the app) — just bring it forward
        // instead of stacking a redundant second tab.
